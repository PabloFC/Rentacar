...
<body>
    <main>
        ...
        if (isset($_SESSION["admin"])) {
            ...
            echo "<h1>Buscar coches</h1>";
        ?>
            <section class="buscar_coches">
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-sm-6 caja_formu">
                            <div class="formu_contacta">
                                <h5 class="color_blanco">Marca/modelo del coche</h5>
                                <br>
        ...
        <?php
            if (isset($_POST["enviar"])) {
                if (!$_POST["buscar"]) {
                    echo '<script>alert("Introduce marca o modelo")</script>';
                } else {

                    $buscar = "%" . $_POST["buscar"] . "%";
                    $sentencia = "select foto,matricula,nombre,modelo,tipo,fecha_compra, precio_dia, caballos, asientos
                    from coche,marca where 
                    coche.marca=marca.codigo and (nombre like ? or modelo like ?)";
                    $consulta = $conexion->prepare($sentencia);
                    $consulta->bind_param("ss", $buscar, $buscar);
                    $consulta->bind_result($fot,$mat,$nom,$mod,$tip,$fe,$pre,$cab,$asi);
                    $consulta->execute();
                    ...
                    if ($consulta->affected_rows == 0) {
                        ...
                        while ($consulta->fetch()) {
                            ...
                            echo "<tr>
                            <td><img style='width:70%' src='../img/$fot'></td><td>$mat</td>
                            <td>$nom</td><td>$mod</td><td>$tip</td><td>$compra</td>
                            <td>$pre €</td><td>$cab</td>
                            <td>$asi</td>
                            </tr>";
                        }
