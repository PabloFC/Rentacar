    ...
    <main>
        ...
        if (isset($_SESSION["admin"])) {
            ...
            while ($fila = $consulta->fetch_array(MYSQLI_ASSOC)) {
                ...
                <input type='text' name='tit' value='$fila[titulo]' class='form-control' id='Titulo'>
            </div>
            <div class='form-group'>
                <label for='Subtitulo'><span class='color_blanco'>Subtitulo</span></label>
                <input type='text' name='sub' value='$fila[subtitulo]' class='form-control' id='Subtitulo'>
            </div>
            <div class='form-group'>
                <label for='Contenido'><span class='color_blanco'>Contenido</span></label>
                <textarea name='con' value='$fila[contenido]' class='form-control' id='Contenido' cols='30' rows='10'></textarea>
            </div>
            <div class='form-group'>
            ...
            if (isset($_POST["enviar"])) {
                if (!($_POST["tit"] && $_POST["sub"] && $_POST["con"] && $_FILES["fot"] && $_POST["fec"])) {
                    ...
                    $sentencia2 = "update noticia set titulo=?,subtitulo=?,contenido=?,foto=?,fecha=? where codigo=$codigo";
                    $consulta2 = $conexion->prepare($sentencia2);
                    $consulta2->bind_param("sssss", $titulo, $subtitulo, $contenido,$foto,$fecha_publicacion);
                    $consulta2->execute();
